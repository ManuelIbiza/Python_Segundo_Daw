...
class Sabana():
    ...
    def caceria(self, depredador:Manada, presa:Manada, despiadada=False):
        ...
        cantidad_dep = len(lista_dep)
        cantidad_pre = len(lista_presa)
        ...
        while contador_duelos < duelos:
            posicion_depred = contador_duelos - muertos_depred
            posicion_presa = contador_duelos - muertos_presa
            lista_dep = depredador.grupo
            lista_presa = presa.grupo
            print(f"\nSe enfrentan {lista_dep[posicion_depred].especie} de {lista_dep[posicion_depred].get_peso()} kilos como depredador y {lista_presa[posicion_presa].especie} de {lista_presa[posicion_presa].get_peso()} kilos como presa.\n")
            lista_dep[posicion_depred].hablar()
            lista_presa[posicion_presa].hablar()

            if lista_dep[posicion_depred].get_peso() > lista_presa[posicion_presa].get_peso():
                lista_presa[posicion_presa].vivo = False
                print(f"\nMuere el {lista_presa[posicion_presa].especie}\n")
                self.muertos.append(lista_presa.pop(posicion_presa))
                muertos_presa += 1
            elif lista_dep[posicion_depred].get_peso() < lista_presa[posicion_presa].get_peso():
                lista_dep[posicion_depred].vivo = False
                print(f"\nMuere el {lista_dep[posicion_depred].especie}\n")
                self.muertos.append(lista_dep.pop(posicion_depred))
                muertos_depred += 1
            else:
